...
def warn_about_dunder_non_retry_state_deprecation(fn, stacklevel):
    msg = '"%s" method must be called with' ' single "retry_state" parameter' % (
    ...
    def new_fn(
        ...
        if retry_state is None:
            ...
            retry_state_passed_as_non_kwarg = (
                previous_attempt_number is not _unset
                and isinstance(previous_attempt_number, RetryCallState)
            )
...
def stop_func_accept_retry_state(stop_func):
    ...
    def wrapped_stop_func(retry_state):
        warn_about_non_retry_state_deprecation("stop", stop_func, stacklevel=4)
        return stop_func(retry_state.attempt_number, retry_state.seconds_since_start,)
...
def wait_func_accept_retry_state(wait_func):
    ...
    if func_takes_last_result(wait_func):
        ...
        def wrapped_wait_func(retry_state):
            warn_about_non_retry_state_deprecation("wait", wait_func, stacklevel=4)
            return wait_func(
        ...
        def wrapped_wait_func(retry_state):
            warn_about_non_retry_state_deprecation("wait", wait_func, stacklevel=4)
            return wait_func(
                retry_state.attempt_number, retry_state.seconds_since_start,
            )
...
def retry_dunder_call_accept_old_params(fn):
    ...
    def new_fn(self, attempt=_unset, retry_state=None):
        if retry_state is None:
            ...
            retry_state_passed_as_non_kwarg = attempt is not _unset and isinstance(
                attempt, RetryCallState
            )
            if retry_state_passed_as_non_kwarg:
                retry_state = attempt
            else:
...
def retry_func_accept_retry_state(retry_func):
    ...
    def wrapped_retry_func(retry_state):
        warn_about_non_retry_state_deprecation("retry", retry_func, stacklevel=4)
...
def before_func_accept_retry_state(fn):
    ...
    def wrapped_before_func(retry_state):
        ...
        return fn(retry_state.fn, retry_state.attempt_number,)
...
def after_func_accept_retry_state(fn):
    ...
    def wrapped_after_sleep_func(retry_state):
        ...
        return fn(
            retry_state.fn, retry_state.attempt_number, retry_state.seconds_since_start
        )
...
def retry_error_callback_accept_retry_state(fn):
    ...
    def wrapped_retry_error_callback(retry_state):
        warn_about_non_retry_state_deprecation("retry_error_callback", fn, stacklevel=4)
