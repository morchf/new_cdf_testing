...
class Publisher:
    ...
    def __publisherThread__(
        ...
        for messageIdx in range(len(messages)):
            if verbose:
                print(
                    f"Client {clientId}:\t\tPublishing message {messageIdx+1} of {totalMessageCount}: {messages[messageIdx]} under topic {topic[messageIdx]}"
                )
            client.publish(topic[messageIdx], messages[messageIdx])
            time.sleep(delay)
