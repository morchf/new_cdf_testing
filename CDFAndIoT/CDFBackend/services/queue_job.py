...
class Client:
    ...
    def error_url(self, QueueUrl):
        job_id = self._id_from_queue_url(QueueUrl)
        return self._error_queue_url_from_id(job_id)
    ...
    def create(
        ...
        SendErrorsToDeadLetterQueue=True,
        Tags={},
        **kwargs,
    ):
        """
        ...
        try:
            ...
            queue = self._create_queue(
                ...
                **(
                    {
                        "RedrivePolicy": {
                            "deadLetterTargetArn": error_queue_arn,
                            "maxReceiveCount": 1,
                        }
                    }
                    if SendErrorsToDeadLetterQueue
                    else {}
                ),
                **kwargs,
            )
            ...
            return {"QueueUrl": queue_url, "FinishTopicArn": finish_topic_arn}
    ...
    def send(self, QueueUrl, Messages):
        """
                ...
                Entries=[
                    {
                        ...
                        "MessageDeduplicationId": f"{queue_id}-{repr(round(time() * 100))}-{i}",
                        "Id": f"{queue_id}-{repr(round(time() * 100))}-{i}",
                    }
    ...
    def errors(self, QueueUrl):
        ...
        while numberOfErrors > 0:
            ...
            errors += [m["Body"] for m in messages]
