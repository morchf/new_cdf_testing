...
def store_cert_data_as_zip(name, prefix):
    ...
    try:
        ...
        zipObj.write(
            os.path.join(
                os.path.abspath(os.path.realpath(__file__) + "/.."), "rootCA.pem"
            ),
            arcname=zip_rootCA_name,
        )
        zipObj.close()
...
def update_configuration_file(name):
    ...
    try:
        file_path = (
            os.path.join(sys.path[0], f"{cnf_name}")
            if (os.path.exists(os.path.join(sys.path[0], f"{cnf_name}")))
            else os.path.join(sys.path[-1], f"{cnf_name}")
        )
