...
def lambda_handler(event, context):
    ...
    # entry validation - should the GPIO Update occur after the TTL has erased the
    # value from the cache, it will result in a malformed entry. Check for this, and
    # treat this as non-existent if encountered.
    cacheVerified = False
    if cache_data.get("Item"):
        if cache_data.get("Item").get("vehicleClass"):
            cacheVerified = True
    if cacheVerified:
        create_topic_and_publish(
            ...
            cache_data["Item"]["vehicleSerialNo"],
            cache_data["Item"]["CMSID"],
            cache_data["Item"]["ID"],
            cache_data["Item"]["GTTSerial"],
            cache_data["Item"]["vehicleMode"],
            cache_data["Item"]["GPIO"],
            cache_data["Item"]["GPIOTimeStamp"],
        )
    else:
        ...
        if template_id == "communicator":
            ...
            try:
                ...
                raise Exception(
                    f"Serial No. is {client_id} Agency Id is {agency_id} CMS Id is {cms_id} MAC address is not numeric"
                )
...
def create_topic_and_publish(
    ...
    # Strictly rounding lat to 2 digits after decimal because the VPS needs it in that format
    ...
    # Strictly rounding lon to 2 digits after decimal because the VPS needs it in that format
    ...
    if live_gpi in GPI_WITH_TURN_SIGNAL and int(message_time) > int(gpio_timestamp):
        cache.update_item(
            Key={"ID": client_id},
            UpdateExpression="set GPIOTimeStamp=:a, GPIO=:b",
            ExpressionAttributeValues={":a": message_time, ":b": live_gpi},
        )
