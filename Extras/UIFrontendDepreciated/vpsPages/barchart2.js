...
import {select, nest, sum, max, scaleLinear, scaleBand, axisBottom, axisLeft} from "d3";
import './barchart.css';
...
class Barchart2 extends Component {
    constructor(props){
        ...
        const serverdata = this.props.vpss.filter(vps => vps.customerName === this.props.customerName);
        this.data = nest()
        .key(function(d) { return d.serverName; })
        .rollup(function(v) {
                return {
                TotalVPC: v.length,
                CountInactive: sum(v, function(e) { return e.deviceStatus === 'INACTIVE'; }),
                CountToDelete: sum(v, function(e) { return e.markToDelete === 'YES'; })
                };
            })
        .entries(serverdata)
        .map(function(group) {
    ...
    draw() {
        ...
        //const width = +this.node.style.width;
        //const height = +this.node.style.height;
        const width = 400;
        const height = 300;
        //console.log(this.node.style['width']);
        //temporary:
        let a = this.props.xaxisname, b = this.props.yaxisname;
        let xValue = d => d[a];
        let yValue = d => d[b];
