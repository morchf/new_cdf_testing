...
class RouteToIntersectionFactory:
    @staticmethod
    def get_name() -> str:
        return "metrics-route-to-intersection"
    ...
    def __call__(self, agency: str, *args, **kwargs) -> Tuple[str, DAG]:
        ...
        with DAG(
            ...
            description=f"Route to Intersection Spark job ({agency})",
            ...
            run_metrics_route_to_intersection_spark_job = EmrAddStepsOperator(
                ...
                steps=[
                    {
                        ...
                        "HadoopJarStep": {
                            ...
                            "Args": [
                                ...
                                f"{artifacts_bucket}/spark/metrics/route_to_intersection.py",
                                "--agency",
                                agency,
                                "--date",
                        ...
                        "Name": f"Run `metrics/route_to_intersection` Spark job ({agency})",
                    }
                ],
                task_id="run_metrics_route_to_intersection_spark_job",
                execution_timeout=timedelta(minutes=60),
            )

            wait_metrics_route_to_intersection_spark_job = EmrStepSensor(
                job_flow_id=job_flow_id,
                aws_conn_id=aws_conn_id,
                step_id="{{ ti.xcom_pull(task_ids='run_metrics_route_to_intersection_spark_job')[0] }}",
                task_id="wait_metrics_route_to_intersection_spark_job",
                execution_timeout=timedelta(hours=1),
            )

            (
                run_metrics_route_to_intersection_spark_job
                >> wait_metrics_route_to_intersection_spark_job
            )

            return dag_id, dag


for dag_id, dag in get_dags_to_load(RouteToIntersectionFactory()):
    globals()[dag_id] = dag
