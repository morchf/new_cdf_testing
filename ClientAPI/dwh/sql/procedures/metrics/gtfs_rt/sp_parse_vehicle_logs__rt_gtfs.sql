CREATE
OR REPLACE PROCEDURE sp_parse_vehicle_logs__rt_gtfs (p_agency TEXT, p_date TEXT) AS $$
DECLARE
  BEGIN
    RAISE INFO 'Parsing vehicle logs from Realtime GTFS and RT Radio data for agency=%, date=%',
    p_agency,
    p_date;
...
-- Create 'vehicle_logs' staging table
CALL sp_vehicle_logs__rt_gtfs(p_agency, p_date);

-- Uncomment to linearly interpolate logs
-- CALL sp_vehicle_logs__interpolated();

/* Remove RT GTFS logs */
DELETE FROM
  staging__vehicle_logs
WHERE
  source_device = 'RT_GTFS';
...
-- DROP TABLE staging__intersection_positions;
-- Create 'segment_events' staging table
