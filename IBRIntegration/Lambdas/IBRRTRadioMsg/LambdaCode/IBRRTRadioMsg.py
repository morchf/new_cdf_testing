...
def connectToRedis():
    try:
        ...
        redis_address = environ["REDIS_URL"]  # redis_cache
        ...
        print(f"[connectToRedis ERROR] - {e}")
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if redis_cache_data:  # redis_cache
            ...
            cache.expire(
                client_id.lower(), 1200
            )  # Update TTL to preserve field in the DB.
        else:
            ...
            if template_id == "communicator":
                ...
                if template_id == "integrationcom":
                    print(
                        f'License state for {client_id} : {str(communicator_attributes.get("preemptionLicense"))}'
                    )
...
def compile_2100_msg(
    ...
    lat = dms_to_min_degrees(lat["degree"], lat["minute"], lat["second"])
    lon = dms_to_min_degrees(lon["degree"], lon["minute"], lon["second"])
    lat = round(float(lat), 6)
    lon = round(float(lon), 6)

    hdg = gps_data["fix"]["heading"]
    spd = gps_data["fix"]["ground_speed_knots"]
    ...
    # io3 = Left
    # io4 = Right
    # io5 = Ignition (if not connected to ignition via the orange wire on the power cable)

    # use GPIO information
    ignition = gps_data["gpio"].get("io5", gps_data["gpio"]["pwrIn"])

    print(f"ignition = {ignition}")
    print(f'Enable = {gps_data["gpio"]["io1"]}')
    print(f'disable = {gps_data["gpio"]["io2"]}')
    print(f'Left = {gps_data["gpio"]["io3"]}')
    print(f'right = {gps_data["gpio"]["io4"]}')
    print(f'io5 = {gps_data["gpio"]["io5"]}')

    left_turn = 0  # (gpi & 0x02) >> 1
    right_turn = 0  # (gpi & 0x04) >> 1  # shift only one for math below

    disable = gps_data["gpio"]["io2"]

    if gps_data["gpio"]["io3"]:
        left_turn = 1

    if gps_data["gpio"]["io4"]:
        right_turn = 2
    ...
    print(f"vehicle_mode = {vehicle_mode}")
    # pack up data
...
def insert_in_cache(
    ...
    cache.expire(client_id.lower(), 1200)
    # redis_cache.hmset(client_id, item)        redis_cache
...
def get_2100_topic(
    ...
    # print (f"_latitude_2_decimal_str = {_latitude_2_decimal_str}")
    # print (f"_lat_dec_count = {_lat_dec_count}")
